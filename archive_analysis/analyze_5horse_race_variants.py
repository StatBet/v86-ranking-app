import pandas as pd

df = pd.read_csv("ml_dataset.csv").fillna(0)

rows = []

for race_id, race in df.groupby("race_id"):
    race = race.sort_values("model_rank")
    winner = race[race["won"] == 1]

    if winner.empty:
        continue

    scores = {int(r["model_rank"]): r["total_score"] for _, r in race.iterrows()}

    r1 = scores.get(1, 0)
    r3 = scores.get(3, 0)
    r4 = scores.get(4, 0)
    r5 = scores.get(5, 0)
    r8 = scores.get(8, 0)

    rows.append({
        "race_id": race_id,
        "date": winner.iloc[0]["date"],
        "race_no": winner.iloc[0]["race_no"],
        "winner": winner.iloc[0]["horse"],
        "winner_rank": int(winner.iloc[0]["model_rank"]),
        "gap_1_3": r1 - r3,
        "gap_1_4": r1 - r4,
        "gap_1_5": r1 - r5,
        "spread_1_8": r1 - r8,
    })

races = pd.DataFrame(rows)

variants = [
    ("gap_1_3 >= 30", races["gap_1_3"] >= 30),
    ("gap_1_3 >= 40", races["gap_1_3"] >= 40),
    ("gap_1_4 >= 30", races["gap_1_4"] >= 30),
    ("gap_1_4 >= 40", races["gap_1_4"] >= 40),
    ("gap_1_4 >= 50", races["gap_1_4"] >= 50),
    ("spread_1_8 >= 70", races["spread_1_8"] >= 70),
    ("spread_1_8 >= 80", races["spread_1_8"] >= 80),
]

print("=" * 80)
print("5-HÄSTARSVARIANTER")
print("=" * 80)

for name, mask in variants:
    subset = races[mask]

    print()
    print(name)
    print("-" * 40)
    print("Lopp:", len(subset))

    if len(subset):
        for n in [3, 4, 5, 6, 8]:
            hits = (subset["winner_rank"] <= n).sum()
            print(f"Rank 1-{n}: {hits}/{len(subset)} = {round(hits / len(subset) * 100, 1)}%")